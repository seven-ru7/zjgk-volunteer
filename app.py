...
if recs:
    ...
    # 学校详情卡片（点击展开查看院校详情）
        with st.expander("🎓 学校详情卡片（点击展开每个志愿）", expanded=False):
            st.caption("每个志愿的院校标签、强势学科、保研/就业率等详情")
            ...
            if tier_filter == "全部":
                ...
                t = tier_filter.split()[-1]
                filtered_recs = [r for r in recs if r.tier == t]

            st.caption(f"显示 **{len(filtered_recs)}** 个志愿")
            ...
            for i in range(0, len(filtered_recs), cols_per_row):
                ...
                for j, r in enumerate(filtered_recs[i:i + cols_per_row]):
                    with cols[j]:
                        p = r.program
                        ...
                        with st.container(border=True):
                            # 第 1 行：序号 + tier + 院校
                            st.markdown(
                                f"**{i+j+1}. {tier_emoji} {p.institution}**",
                                help=p.name
                            )
                            # 第 2 行：tier 标签徽章（紧凑）
                            tags_html = " ".join(
                                f'<span style="background:#{"c00" if t=="985" else "06c" if t=="211" else "060" if "双一流" in t else "666"};color:white;padding:1px 6px;border-radius:3px;font-size:10px;margin-right:2px;font-weight:bold">{t}</span>'
                            ...
                            c1, c2 = st.columns(2)
                            c1.metric("📊 保研", postgrad.replace("约 ", ""))
                            c2.metric("💼 就业", employ.replace("约 ", ""))

                            # 第 4 行：趋势 + 概率
                            st.markdown(
                                f"**{card['sparkline']}** &nbsp;{card['trend']['direction']} &nbsp;· "
                                f"{tier_emoji} **{prob}** · {card['stability']}"
                            )

                            # 第 5 行：强项学科（精简）
                            if detail.get("strong_majors"):
                                majors_str = " · ".join(detail["strong_majors"][:3])
                                st.markdown(f"🎯 {majors_str}")

                            # 详情放 popover（点击展开，避免页面拥挤）
                            with st.popover("📖 详细信息"):
                                st.markdown(f"**{p.institution}** · {p.name}")
                                st.markdown(f"📍 {p.city or '—'} · 💰 {p.tuition} 元/年 · ⏱️ {p.duration_years} 年")
                                ...
                                if p.required_subjects:
                                    st.markdown(f"**📋 选考要求**：{'、'.join(p.required_subjects)}")
                                else:
                                    st.caption("📋 选考要求：不限")
